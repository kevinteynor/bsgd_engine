...
#pragma endregion
	...
	QCheckBox* discardShaderCbx;
	mainLayout->addWidget(discardShaderCbx = new QCheckBox("Discarded Smile shader"));
	connect(discardShaderCbx, SIGNAL(toggled(bool)), this, SLOT(discardCbxTicked()));
...
void MyMainWindow::discardCbxTicked()
{
	renderWindow->discardShader = !renderWindow->discardShader;
	renderWindow->updateShaders();
}