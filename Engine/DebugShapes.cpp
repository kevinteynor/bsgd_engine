...
uint DebugShapes::MakeArrow(const glm::mat4& position, float lifetime, bool depthTest, const glm::vec3& color)
{
	Assert(activeRenderer != NULL);
...
void DebugShapes::MakeCrosshair(const glm::mat4& position, float lifetime, bool depthTest)
{
	Assert(activeRenderer != NULL);
	
	MakeLine(position * glm::scale(-1.0f, 1.0f,1.0f) * glm::translate(-1.0f, 0.0f, 0.0f), lifetime, depthTest, glm::vec3(1.0f, 0.0f, 0.0f));	// x-axis
	MakeLine(glm::translate(0.0f, 1.0f, 0.0f) * glm::rotate(position, -90.0f, 0.0f, 0.0f, 1.0f), lifetime, depthTest, glm::vec3(0.0f, 1.0f, 0.0f)); // y-axis
	MakeLine(glm::translate(0.0f, 0.0f, 1.0f) * glm::rotate(position, 90.0f, 0.0f, 1.0f, 0.0f), lifetime, depthTest, glm::vec3(0.0f, 0.0f, 1.0f));	// z-axis
}
uint DebugShapes::MakeCube(const glm::mat4& position, float lifetime, bool depthTest, const glm::vec3& color)
{
	Assert(activeRenderer != NULL);
...
uint DebugShapes::MakePlane(const glm::mat4& position, float lifetime, bool depthTest, const glm::vec3& color, uint dimensions)
{
	Assert(activeRenderer != NULL);

	if(PLANE_GEO_ID == (uint)-1)
	{
		Neumont::ShapeData data = Neumont::ShapeGenerator::makePlane(dimensions);
...
uint DebugShapes::MakeSphere(const glm::mat4& position, float lifetime, bool depthTest, const glm::vec3& color, uint tesselation)
{
	Assert(activeRenderer != NULL);
...
uint DebugShapes::MakeShape(DebugShapeInstanceInfo& shapeInfo)
{
	...
	shapeInstances[shapeInstances.size() - 1].colorUniformIndex = shapeInstances[shapeInstances.size() - 1].renderable->AddUniform("uniColor", ut_VEC3, &shapeInfo.color, sizeof(glm::vec3));
