...
{
public:
	struct ENGINE_SHARED DebugShapeInstanceInfo
	{
		bool UseDepthTest;
		uint geoBufferID;
		float duration;
...
public:
	...
	static uint MakeArrow(const glm::mat4& position, float lifetime = -1, bool depthTest = true, const glm::vec3& color = glm::vec3(1.0f,0.0f,0.0f));
	static void MakeCrosshair(const glm::mat4& position, float lifetime = -1, bool depthTest = true);
	static uint MakeCube(const glm::mat4& position, float lifetime = -1, bool depthTest = true, const glm::vec3& color = glm::vec3(1.0f,0.0f,0.0f));
	static uint MakeLine(const glm::mat4& position, float lifetime = -1, bool depthTest = true, const glm::vec3& color = glm::vec3(1.0f,0.0f,0.0f));
	static uint MakeVector(const glm::vec3& start, const glm::vec3& end, float lifetime = -1, bool depthTest = true, const glm::vec3& color = glm::vec3(1.0f, 0.0f, 0.0f));
	static uint MakePlane(const glm::mat4& position, float lifetime = -1, bool depthTest = true, const glm::vec3& color = glm::vec3(1.0f,0.0f,0.0f), uint dimensions = 10);
	static uint MakeSphere(const glm::mat4& position, float lifetime = -1, bool depthTest = true, const glm::vec3& color = glm::vec3(1.0f,0.0f,0.0f), uint tesselation = 10);
