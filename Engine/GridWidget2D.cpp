...
GridWidget2D::GridWidget2D(void)
{
	ClickAndDrag = false;
	watchedPoints = 0;
...
void GridWidget2D::paintEvent(QPaintEvent* e)
{
	e;
...
PointWatchInfo* GridWidget2D::WatchPoint(float* x, float* y, const float* imass, Qt::GlobalColor color, bool circle)
{
...
AttachedVectorWatchInfo* GridWidget2D::WatchAttachedVector(const float* x, const float* y, const float* x_origin, const float* y_origin, Qt::GlobalColor color)
{
	if(watchedPoints >= MAX_WATCHES)
		return 0;
...
void GridWidget2D::wheelEvent(QWheelEvent *e)
{
...
void GridWidget2D::mousePressEvent(QMouseEvent *e)
{
...
void GridWidget2D::mouseMoveEvent(QMouseEvent *e)
{
...
void GridWidget2D::mouseReleaseEvent(QMouseEvent *e)
{
	e;
	if(ClickAndDrag && draggingPoint)
		draggingPoint = 0;
...
PointWatchInfo* GridWidget2D::GetClosestPointWatchInfo(glm::vec2 p)
{
	p -= gridOriginOffset;
	p /= gridLineSpacing;
	p = glm::vec2(p.x, -p.y);
