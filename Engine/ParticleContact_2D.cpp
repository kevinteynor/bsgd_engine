...
ParticleContact_2D::ParticleContact_2D(TPhysics::Particle_2D* a, TPhysics::Particle_2D* b, float restitution)
	: p1(a), p2(b), restitution(restitution){}

float ParticleContact_2D::calculateSeparatingVelocity() const
{
...
void ParticleContact_2D::Resolve()
{
	resolveVelocity();
...
void ParticleContact_2D::resolveInterpenetration()
{
...
void ParticleContactResolver_2D::SetIterations(uint i)
{
	iterations = i;
}
void ParticleContactResolver_2D::ResloveContacts(ParticleContact_2D * contactArray, uint numContacts)
{
	uint i;
